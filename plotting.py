...
def plot_var(f_step, projection):
  ...
  filename = '/tmp/' + projection + '_' + variable_name + '_%s_%03d.png' % (run_string, f_step)
...
def plot_vars(f_step, projection, load_all=False):
  ...
  filenames = ['/tmp/' + projection + '_' + variable_name + '_%s_%03d.png' % (run_string, f_step) for f_step in f_steps]
  ...
  if 'valid_time' not in dset.dims.keys():
    dset = dset.expand_dims('valid_time')
  # 
  dset = subset_arrays(dset, projection)
  time = pd.to_datetime(dset.valid_time.values)
  cum_hour = dset.step.values.astype(int)
...
def single_plot(it, **args):
  ...
  fig = plt.figure(figsize=(figsize_x, figsize_y))
  ax = plt.gca()
  ax = get_projection_cartopy(plt, args['projection'], compute_projection=True)
  ...
  an_var = annotation(
    ax, 'Geopotential height @500hPa [m] and temperature @850hPa [C]', loc='lower left', fontsize=6)
  an_run = annotation(ax, 'Run: '+args['run_string']+' UTC', loc='upper right')
